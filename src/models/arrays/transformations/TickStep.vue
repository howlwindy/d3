...
const result0 = ref<number>(d3.tickStep(data0.value, data1.value, data2.value))
const input0 = (v: number) =>
  (result0.value = d3.tickStep(v, data1.value, data2.value))
const input1 = (v: number) =>
  (result0.value = d3.tickStep(data0.value, v, data1.value))
const input2 = (v: number) =>
  (result0.value = d3.tickStep(data0.value, data1.value, v))
</script>
<template>
  <GroupItemVue>
    <h3><code>tickStep - 步长</code></h3>
    ...
    <EgVue>
      ...
      <CodeVue :result="result0">
        <code>d3.tickStep({{ data0 }}, {{ data1 }}, {{ data2 }})</code>
      </CodeVue>
    </EgVue>
  </GroupItemVue>
</template>
