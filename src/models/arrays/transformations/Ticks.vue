...
const result0 = ref<number[]>(d3.ticks(data0.value, data1.value, data2.value))
const input0 = (v: number) =>
  (result0.value = d3.ticks(v, data1.value, data2.value))
const input1 = (v: number) =>
  (result0.value = d3.ticks(data0.value, v, data1.value))
const input2 = (v: number) =>
  (result0.value = d3.ticks(data0.value, data1.value, v))
</script>
<template>
  <GroupItemVue>
