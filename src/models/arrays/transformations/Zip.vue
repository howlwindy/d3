...
const src = ref<string[]>([
  'zip<string[]>(...arrays: ArrayLike<string[]>[]): string[][][]'
])

const data0 = ref<string>('a 2 10 -1 -2')
const result0 = ref<string[][][]>(
  d3.zip(strToStrArr(data0.value).map((v) => [v]))
)
const input0 = (v: string) =>
  (result0.value = d3.zip(strToStrArr(v).map((v) => [v])))
</script>
<template>
  <GroupItemVue>
    <h3><code>zip - ?</code></h3>
    ...
    <EgVue>
      ...
      <CodeVue :result="result0">
        <code>d3.zip({{ strToStrArr(data0).map((v) => [v]) }})</code>
      </CodeVue>
    </EgVue>
  </GroupItemVue>
</template>
