...
const result0 = ref<boolean>(d3.disjoint(data0.value, '*'))
const input0 = (v: string) => (result0.value = d3.disjoint(v, '*'))
</script>
<template>
  <GroupItemVue>
    <h3><code>disjoint - 断言两个集合是否不相交</code></h3>
    <SrcVue :data="src"></SrcVue>
    <EgVue>
      <input v-model="data0" type="text" @input="input0(data0)" />
      <CodeVue :result="result0">
        <code>d3.disjoint({{ data0 }}, '*')</code>
      </CodeVue>
    </EgVue>
  </GroupItemVue>
</template>
