...
const result0 = ref<string | undefined>(
  d3.greatest(data0.value.split(' '), (a, b) => a.length - b.length)
)
const input0 = (v: string) =>
  (result0.value = d3.greatest(v.split(' '), (a, b) => a.length - b.length))

const data1 = ref<string>('a 2 10 -1 -2')
const result1 = ref<string | undefined>(
  d3.greatest(data1.value.split(' '), (a) => a.length)
)
const input1 = (v: string) =>
  (result1.value = d3.greatest(v.split(' '), (a) => a.length))
</script>
<template>
  <GroupItemVue>
    <h3><code>greatest - 符合条件的最大元素</code></h3>
    <SrcVue :data="src"></SrcVue>
    <EgVue>
      <input v-model="data0" type="text" @input="input0(data0)" />
      <CodeVue :result="result0">
        <code>
          d3.greatest({{ data0.split(' ') }}, (a, b) => a.length - b.length)
        </code>
      </CodeVue>
    </EgVue>
    <EgVue>
      ...
      <CodeVue :result="result1">
        <code>d3.greatest({{ data1.split(' ') }}, a => a.length</code>
      </CodeVue>
    </EgVue>
  </GroupItemVue>
</template>
