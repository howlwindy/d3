...
const result0 = ref<number>(d3.thresholdScott(strToNumArr(data0.value), 0, 100))
const input0 = (v: string) =>
  (result0.value = d3.thresholdScott(strToNumArr(v), 0, 100))
</script>
<template>
  <GroupItemVue>
    <h3><code>thresholdScott - ?</code></h3>
    <SrcVue :data="src"></SrcVue>
    <EgVue>
      <input v-model="data0" type="text" @input="input0(data0)" />
      <CodeVue :result="result0">
        <code>d3.thresholdScott({{ strToNumArr(data0) }}, 0, 100)</code>
      </CodeVue>
    </EgVue>
  </GroupItemVue>
</template>
