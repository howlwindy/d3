...
const src = ref<string[]>([
  'variance(iterable: Iterable<Numeric | undefined | null>): number | undefined',
...
const result0 = ref<number | undefined>(d3.variance(strToNumArr(data0.value)))
const input0 = (v: string) => (result0.value = d3.variance(strToNumArr(v)))

const data1 = ref<string>('a 2 10 -1 -2')
const result1 = ref<number | undefined>(
  d3.variance(strToNumArr(data1.value), (datum) => datum)
)
const input1 = (v: string) =>
  (result1.value = d3.variance(strToNumArr(v), (datum) => datum))
</script>
<template>
  <GroupItemVue>
    <h3><code>variance - 方差</code></h3>
    <SrcVue :data="src"></SrcVue>
    <EgVue>
      <input v-model="data0" type="text" @input="input0(data0)" />
      <CodeVue :result="result0">
        <code>d3.variance({{ strToNumArr(data0) }})</code>
      </CodeVue>
    </EgVue>
    <EgVue>
      <input v-model="data1" type="text" @input="input1(data1)" />
      <CodeVue :result="result1">
        <code>d3.variance({{ strToNumArr(data1) }}, (datum) => datum</code>
      </CodeVue>
    </EgVue>
  </GroupItemVue>
</template>
