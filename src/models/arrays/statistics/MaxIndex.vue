...
const result0 = ref<number | undefined>(d3.maxIndex(strToStrArr(data0.value)))
const input0 = (v: string) => (result0.value = d3.maxIndex(strToStrArr(v)))
  ...
  <GroupItemVue>
    ...
    <EgVue>
      ...
      <CodeVue :result="result1">
        <code>
          d3.maxIndex({{ strToStrArr(data1) }}, (datum, index, array) =>
          `${datum} - ${index} - ${array}`)
        </code>
      </CodeVue>
    </EgVue>
