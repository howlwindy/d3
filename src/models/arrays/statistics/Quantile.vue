...
const result0 = ref<number | undefined>(
  d3.quantile(strToNumArr(data0.value), +data01.value)
)
const input0 = (v: string, p: string) =>
  (result0.value = d3.quantile(strToNumArr(v), +p))

const data1 = ref<string>('a 2 10 -1 -2')
const data11 = ref<string>('0.5')
const result1 = ref<number | undefined>(
  d3.quantile(strToNumArr(data1.value), +data11.value, (element) => element)
)
const input1 = (v: string, p: string) =>
  (result1.value = d3.quantile(strToNumArr(v), +p, (element) => element))
</script>
<template>
  <GroupItemVue>
    ...
    <EgVue>
      ...
      <CodeVue :result="result0">
        <code>d3.quantile({{ strToNumArr(data0) }}, {{ data01 }})</code>
      </CodeVue>
    </EgVue>
    <EgVue>
      ...
      <CodeVue :result="result1">
        <code>
          d3.quantile({{ strToNumArr(data1) }}, {{ data11 }}, (element) =>
          element
        </code>
      </CodeVue>
    </EgVue>
